...
class InstagramBot:
    ...
    def execute(data = None):
        ...
        Parameters : 
            INPUT : count_break -> total number of likes in one go
                    default = 150
        """
        count = 0
        for index in data.index:
            user = data.loc[index, "users"]
            interaction_status = data.loc[index, 'interaction_status']
            if interaction_status == 0:
                insta.profile(user)
                privacy = insta.accountPrivacy()
                data.loc[index, 'interaction_status'] = 1
                if privacy == False:
                    post_count = insta.countPosts()
                    if post_count not in ["0", "1", "2", "3"]:
                        insta.likeStory()
                        insta.likePhotos()
                        data.loc[index, "pic_like_status"] = True
                        data.loc[index, 'pic_like_date'] = date.today()
                        # saving the data back to csv 
                        data.to_csv("data/UserList.csv", index=None)
                        count+=1
                        if (count % 10 == 0):
                            print(count)
                        if (count > 150):
